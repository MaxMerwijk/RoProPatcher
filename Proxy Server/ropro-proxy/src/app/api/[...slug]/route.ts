...
export async function GET(
  ...
  return handleProxyRequest(request, params.slug);
}
...
async function handleProxyRequest(request: NextRequest, slug: string[]) {
  try {
    // Construct the target URL
    const path = slug.join('/');
    const targetUrl = `https://api.ropro.io/${path}`;
    ...
    // Set the host header to api.ropro.io
    headers.set('host', 'api.ropro.io');
